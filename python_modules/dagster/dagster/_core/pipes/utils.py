...
from typing import IO, Dict, Iterator, Mapping, Optional, Sequence, Tuple, TypeVar
...
class PipesThreadedMessageReader(PipesMessageReader):
    """A base class for MessageReaders which are supposed to read messages and logs in background threads.

    Args:
        interval (float): The interval in seconds at which to poll for messages.
        log_readers (List[PipesLogReader]): A list of log readers to use to read logs.
    """
    ...
    def __init__(
        ...
        self.log_readers = {  # this converts a Mapping to Dict (self.log_readershas to be mutable in order to add extra readers)
            name: reader
    ...
    @abstractmethod
    def are_messages_readable(self, params: PipesParams) -> bool: ...
    ...
    def _messages_thread(
        ...
        try:
            ...
            while True:
                ...
                # always check for new params in case they have been updated
                params = {**params, **(self.launched_payload or {})}

                if not can_read_messages:
                    can_read_messages = self.are_messages_readable(params)
    ...
    def _logs_thread(
        ...
        log_params = {**params, **(self.opened_payload or {})}
        ...
        try:
            ...
            while True:
                log_params = {**log_params, **(self.opened_payload or {})}
                ...
                for key in list(unstarted_log_readers.keys()).copy():
                    if unstarted_log_readers[key].are_logs_readable(log_params):
